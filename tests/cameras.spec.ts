...
test.describe('Camera Selection and Video', () => {
  ...
  test('should restore camera selection from URL on fresh navigation', async ({ page }) => {
    await gotoAuthenticated(page)
    ...
    // Navigate to the captured URL with camera ID (dev-bypass re-authenticates on each load)
    console.log(`Re-navigating to URL: ${urlWithCamera}`)
    await gotoAuthenticated(page, urlWithCamera.replace(/^https?:\/\/[^/]+/, ''))

    // After navigation, URL should contain the camera ID
    ...
    await expect(mainVideoPlayer).toBeVisible({ timeout: TIMEOUTS.VIDEO_LOAD })
    await expect(mainVideoPlayer).toHaveAttribute('data-camera-id', targetCameraId || '', { timeout: TIMEOUTS.VIDEO_LOAD })
    console.log(`Camera ${targetCameraId} is correctly selected after navigation`)
